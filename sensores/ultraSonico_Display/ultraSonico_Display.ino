...
// Ou seja, 343 metros/segundo = 34300 cm/segundo = 0.0343 cm/microssegundo
...
//------------------------------------------------------------------------------------------------------
  ...
  pinMode(pinoTrig, OUTPUT); // O pino Trig envia o pulso
  pinMode(pinoEcho, INPUT);  // O pino Echo recebe o pulso de volta
}

void loop() {
  // Limpa o pino Trig, garantindo que não há pulso residual
  ...
  // A função pulseIn() retorna a duração do pulso em microssegundos
  long duracao = pulseIn(pinoEcho, HIGH);

  // Calcula a distância usando a fórmula
  // A distância é (tempo de ida e volta * velocidade do som) / 2
  ...
  // Pequeno atraso para não lotar o monitor serial com leituras
  delay(500); // Espera 0.5 segundo antes da próxima leitura
